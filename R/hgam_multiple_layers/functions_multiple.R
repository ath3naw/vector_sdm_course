...
partial_complex <- function(var, means, bounds, cons, minmax, max_average_catch_size=5000, length_out = 25, cp){
  ...
  pr_data <- seq(from=bounds$min.min[i],
                 to=bounds$min.max[i],
                 length.out=25)
  gamma <- rep(beta_group["int"]+complex_data[cp,]$int, length_out)
  for(term in names(beta_group)){
    if(term == "int") next # skip intercept
    # if name of variable ends in 2
    if(grepl("[0-9]$", term)){
      ...
      if(base_var == var_name){
        if(base_var %in% names(cons)){
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * (pr_data-cons[base_var])^suffix
        }else{
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * pr_data^suffix
        }
      }else{
        if(base_var %in% names(cons)){
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * (covs_means[[base_var]]-cons[base_var])^suffix
        }else{
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * covs_means[[base_var]]^suffix
        }
      }
    }else{
      if(term == var_name){
        gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * pr_data
      }else{
        gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]) * covs_means[[term]]
      }
    }
  }
  
  gamma_pr <- exp(gamma)
  scaled <- gamma_pr-minmax[cp,1]+0.00001
  scaled <- scaled/(minmax[cp,2]-minmax[cp,1]+0.00001)
  prob_pres <- probability_of_presence(scaled, max_average_catch_size)
...
partial_spec <- function(var, means, bounds, cons, minmax, max_average_catch_size=5000, length_out = 25, cp, sp){
  ...
  pr_data <- seq(from=bounds$min.min[i],
                 to=bounds$min.max[i],
                 length.out=25)
  name_cp <- paste0("cp", cp)
  name_sp <- paste0("sp", sp)
  spec_row <- which(species_data$complex == name_cp & species_data$species == name_sp, )
  id <- species_data$species_id[spec_row]
  gamma <- rep(beta_group["int"]+complex_data[cp,]$int+species_data[id,]$int, length_out)
  for(term in names(beta_group))  {
    if(term == "int") next # skip intercept
    # if name of variable ends in 2
    if(grepl("[0-9]$", term)){
      ...
      if(base_var == var_name){
        if(base_var %in% names(cons)){
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * (pr_data-cons[base_var])^suffix
        }else{
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * pr_data^suffix
        }
      }else{
        if(base_var %in% names(cons)){
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * (covs_means[[base_var]]-cons[base_var])^suffix
        }else{
          gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * covs_means[[base_var]]^suffix
        }
      }
    }else{
      if(term == var_name){
        gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * pr_data
      }else{
        gamma <- gamma + (beta_group[term]+complex_data[cp,][[term]]+species_data[id,][[term]]) * covs_means[[term]]
      }
    }
  }
  
  gamma_pr <- exp(gamma)
  scaled <- gamma_pr-minmax[id,1]+0.00001
  scaled <- scaled/(minmax[id,2]-minmax[sp,1]+0.00001)
  prob_pres <- probability_of_presence(scaled, max_average_catch_size)
...
generate_data_tabular <- function(n_samples, bias, prob_pres, n_sp, weighted = FALSE){
  ...
  pa_tabular <- pa_df %>%
    ...
    relocate(x, y,
             .after = site_id)
  species_cols <- as.character(1:num_sp)
  pa_tab <- pa_tabular
  j <- 0
  for(i in 1:n_cp){
    colname <- paste0("complex", i)
    pa_tab[[colname]] <- as.integer(rowSums(
      pa_tab[, species_cols[(1+j):(n_sp[i,]+j)]])>0
    )
    j <- j+n_sp[i,]
  }
  pa_tab <- pa_tab %>%
    mutate(group = ifelse(rowSums(across(all_of(species_cols)))>0, 1, 0))
  pa_tab
}
...
generate_model_data <- function(n_samples, n_group, n_complex, n_cp, n_sp, pa_tab){
  pa_tab_1 <- pa_tab[1:n_group,] |>
    ...
    rename(pa = group) |>
    mutate(sp = "group")
  n_complex_l <- n_complex-n_group
  
  j <- n_group
  pa_tab_2 <- data.frame()
  for(i in 1:n_cp){
    if(n_complex_l[i] > 0){
    name <- paste0("complex", i)
      pa_tab_2 <- rbind(pa_tab_2, pa_tab[(1+j):(n_complex_l[i]+j),] |>
                          select(site_id, x, y, name) |>
                          rename(pa = name) |>
                          mutate(sp = name)
      )
    }
  ...
  num_cp <- sum(n_complex_l)
  num_species <- sum(n_sp)
  complex_names <- paste0("complex", 1:n_cp)
  sp_names <- as.character(1:num_species)
  
  pa_tab_3 <- pa_tab[(num_cp+n_group+1):n_samples,] |>
    select(-all_of(c("group", complex_names))) |>
    pivot_longer(cols = sp_names, names_to = "sp", values_to = "pa")
  
  pa_long <- rbind(pa_tab_1, pa_tab_2, pa_tab_3)
  j <- 0
  for(i in 1:n_cp){
    pa_long <- pa_long |> 
    ...
    pa_long <- pa_long |>
      mutate(not_complex = ifelse(sp %in% c(complex_names, "group"), 0, 1))
  covs_vals <- extract(x=covs, y=pa_long |> select(x, y))
  covs_vals <- covs_vals |> select(-any_of(c("sp","not_complex", complex_names)))
